...
public extension Date {
    ...
    func toString(CustomFormat: String, timeZone: TimeZoneType = .local, locale: Locale = Locale.current) -> String {
        //        let offset = Foundation.NSTimeZone.default.secondsFromGMT() / 3600
        //        let nowMillis = 1000 * self.timeIntervalSince1970
        //        return String(format: CustomFormat, nowMillis, offset)
        
        let formatter = Date.cachedFormatter(CustomFormat, timeZone: timeZone.timeZone, locale: locale)
        return formatter.string(from: self)
    }
    ...
    var formattedTimeAgo: String {
        ...
        let components = Calendar.current.dateComponents(set, from: Date(), to: self)
...
public enum DateFormatType {
    ...
    var stringFormat:String {
        switch self {
        case .isHoursMin: return "hh:mm a"
        case .isoYear: return "yyyy"
        case .isoYearMonth: return "yyyy-MM"
        case .isoDate: return "yyyy-MM-dd"
        case .serverDate: return "yyyy/MM/dd"
