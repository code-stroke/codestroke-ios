...
//  Pods
//
//  Created by Jayesh on 17/2/2016.
//  Copyright (c) 2016 Jayesh. All rights reserved.
//
import Foundation
import UIKit
...
@objc public protocol GrowingTextViewDelegate: UITextViewDelegate {
    ...
    @objc optional func textViewGrowingDidEndEditing(_ textView: GrowingTextView)
    @objc optional func textViewGrowingDidChange(_ textView: GrowingTextView)
}
...
open class GrowingTextView: UITextView {
    ...
    @objc func associateConstraints() {
        ...
        if let notificationObject = notification.object as? GrowingTextView {
            if notificationObject === self {
                if trimWhiteSpaceWhenEndEditing {
                    ...
                    if let delegate = delegate as? GrowingTextViewDelegate {
                        delegate.textViewGrowingDidEndEditing!(self)
                    }
                    setNeedsDisplay()
                }
            }
    ...
    @objc func textDidChange(notification: Notification) {
        if let delegate = delegate as? GrowingTextViewDelegate {
            delegate.textViewGrowingDidChange!(self)
        }
